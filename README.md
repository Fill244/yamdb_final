...
```bash
git clone https://github.com/Fill244/yamdb_final.git
```
...
```
- перенести redoc.yaml из templates в static:
```
docker-compose exec web mv templates/redoc.yaml static/
```
...
Подробные примеры запросов и коды ответов приведены в прилагаемой документации в формате ReDoc http://51.250.92.52/redoc/
